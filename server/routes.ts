...
export const renderInstruction = (project: VideoProject, renderer: string): string =>
  [
    ...
    `Then read it in full (channel.get_project ${project.id}) and render exactly this plan${project.kind === "generation" ? " — every scene in order, as one generate_video call in its model and style template" : " — the moment named in each source"}.`,
