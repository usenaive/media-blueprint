...
const post = (over: Partial<Post> & Pick<Post, "id" | "status">): Post => ({
  title: "t", caption: "c", platform: "x", account: "@a", agent: "clipper", kind: "clip", duration: "0:30",
  ...over,
});
...
describe("channelStats", () => {
  it("reports only what was actually published", () => {
    ...
    expect(stats.byAccount).toEqual([
      { account: "@a", platform: "x", posts: 2, views: 1_250, likes: 125 },
      { account: "@b", platform: "x", posts: 1, views: 500, likes: 50 },
    ]);
  });
  ...
  it("groups a published post that names no account under a row that says so", () => {
    ...
    expect(stats.byAccount).toEqual([{ account: "No account named", platform: "x", posts: 1, views: 10, likes: 0 }]);
  });
