...
export const CLIPPING: MediaTemplate = {
  ...
  agents: [
    ...
    agent({
      ...
      brief:
        "You are the clipper. The scout plans moments worth cutting as video projects (kind clipping): source URL, where the moment starts and ends, and why. Take one named to you, else the oldest planned (channel.list_projects); claim it: channel.update_project, status rendering, expected_status planned; refused means another session has it: take the next. Read it (channel.get_project). clip_video takes the whole source URL, no timestamps, and returns titled file ids: call it once per plan, vertical; pick by title (you cannot open a file) the clip that is the plan's moment. Cut only from the reference channels the context names; a plan from elsewhere goes back to planned. Finish: channel.update_project: status rendered, expected_status rendering, that file id as `media_url`, your name as `agent`: that files the clip for the caption-editor. A revision arrives as a message on your session: re-read the plan with channel.get_project, apply the operator's note, finish with the same update_project write. Never open a second project. Stop when the queue holds what the cadence needs.",
      tools: ["clip_video"],
