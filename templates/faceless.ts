...
export const FACELESS: MediaTemplate = {
  ...
  agents: [
    ...
    agent({
      ...
      // `clip_video` is here for the `reference-study` card and nothing else. It is the only tool
      // on this platform that can actually watch a video — it downloads a public URL, transcribes
      // and scores it, and hands back the moments it rated highest — so without it "study the
      // reference" degrades to reading a page about it. It is used once per install, on day one;
      // the ongoing crons never call it, and the brief above never mentions it.
      tools: ["web_search", "web_fetch", "clip_video"],
  ...
  tasks: [
    ...
    task({
      ...
      body: "Read project_context. If it names no reference, file nothing, close this card with a note saying there is no reference and that the team works from the niche alone, and stop — that is a complete answer to this card and the seats behind it open immediately. If it names one or several, study them and file ONE reference teardown. For a single video URL, clip_video is how you actually watch it: call it once on the URL, vertical, and read the titles of the clips it returns — they are the moments it scored highest, which is a reading of the pacing you cannot get from a page. For a channel or handle, web_search and web_fetch it: the titles it publishes, the descriptions, the first lines, what the comments say it does well. Then write the teardown as a pending post with no media and no stage, `source` \"reference teardown\", and make it specific enough to plan from: the hook patterns it uses and the exact words of two or three of them; what it does in the first three seconds; how fast it cuts and how many shots a piece runs to; the shot grammar, what the camera does; who the narrator is and at what pace; the caption and hashtag shape; the formats it repeats, named, because the scout briefs against those names; and what it never does. Where you are reading a page rather than a video, say which parts are your inference and which you actually saw — a teardown that guesses confidently is worse than one that is short. Put the post's id in the note. Do not write hooks, scripts or briefs here: the cards behind this one do that, and they read what you filed.",
    }),
    task({
      key: "look",
