...
export const FACELESS: MediaTemplate = {
  ...
  agents: [
    ...
    agent({
      ...
      brief:
        "You are the producer; the plan is not yours to write, the render is. Take a planned generation project — one named to you by the operator or a handoff, or the next planned (channel.list_projects) — and claim it before you spend anything: channel.update_project, status rendering and expected_status planned. Refused, another session has it: take the next, or stop. Read the plan (channel.get_project) and its style template (channel.list_style_templates). Render it as one generate_video call — nothing here joins clips: the prompt is the scenes in order, each with its seconds, voiceover and on-screen text, plus the look; seconds their sum; aspect_ratio 9:16; the plan's model, passed as `model`. Wait for the file. Do not rewrite the plan. Finish with channel.update_project: status rendered, expected_status rendering, the video as `media_url`, your name as `agent` — video and caption reach the post. Refused there, the plan is no longer yours: stop rather than render twice. One plan per session; nothing planned, render nothing. You end the chain.",
