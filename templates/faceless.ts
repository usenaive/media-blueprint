...
export const FACELESS: MediaTemplate = {
  ...
  agents: [
    ...
    agent({
       ...
       *
       * *** `browser` IS DELIBERATELY NOT HERE, THOUGH IT WOULD HELP. *** Its screenshot now
       * returns the picture rather than a file id, so a channel PAGE could be looked at. But a
       * browser is a seat that can navigate, fill forms and act on any site, and every other seat
       * of both templates is denied it by name (`templates.test.ts` holds that for all of them).
       * Buying a page screenshot with a general-purpose browser on a content agent is the wrong
       * trade, and the thing it would show — thumbnails and titles — is worth much less than one
       * still of the video itself. The honest remedy is the question's own help text: give stills.
       */
